...
--
-- In the library HaskellForMaths exists a function that performs the same (%%) so
-- we can check the results. For example:
-- λ> clean (x1^3:: LexPoly F2 String)
-- x1
-- λ> (x1^3:: LexPoly F2 String) %% [x1^2+x1]
-- x1
-- λ> clean (x1^3*x2^6+x3^2*x4+x1+1 :: LexPoly F2 String)
-- x1x2+x1+x3x4+1
-- λ> (x1^3*x2^6+x3^2*x4+x1+1 :: LexPoly F2 String) %%
...
bind3 m v =
  if (elem varDif mIndices)
  then (product [(var x) ^ i | (x,i) <- mIndices, x /= fst varDif])
  else 0
...
--
-- λ> deriv (x1:: LexPoly F2 String) (x1:: LexPoly F2 String)
-- 1
-- λ> deriv (1+x1+x2+x1*x2:: LexPoly F2 String) (x1:: LexPoly F2 String)
-- x2+1
-- λ> deriv (x1*x2+x1+x3*x4+1:: LexPoly F2 String) (x1:: LexPoly F2 String)
-- x2+1
...
deriv f v = linear (\m -> bind3 m v) f
